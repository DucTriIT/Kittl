...
function insertControlsHtml() {
  ...
  <div id="loading">
  <img id="loading-image" src="${chrome.runtime.getURL("ajax-loader.gif")}" alt="Please do not close this tab while app is running..." />
  </div>`;
...
chrome.runtime.onMessage.addListener(async function (event) {
       ...
       var mutationObserver = new MutationObserver(function (mutations) {
         mutations.forEach(function (mutation) {
           ...
           if (!mutation || !mutation.target) {
             return;
           } else if (mutation.target.innerText && mutation.target.innerText == 'Download') {
             click(downloadButton);
             ...
             if (event.message.fileType === 'PNG') {
               try {
                 ...
                 if(transparentDiv){
                   var transparentButton = validate(transparentDiv.querySelector(event.message.pngTransparency?extensionConfig.transparentSwitcher:extensionConfig.notTransparentSwitcher), 'transparency button');
             ...
             let fileType = event.message.fileType.indexOf(' ') != -1 ? event.message.fileType.split(' ')[0].toLowerCase() : event.message.fileType.toLowerCase(); // Tell background to change the file name before saving it.
             ...
             chrome.runtime.sendMessage({
               ...
               try {
                var closeButton = validate(document.querySelector(extensionConfig.closeButton), 'close button');
                if(closeButton){
                 click(closeButton);
                }
                startedDownload = false;
  ...
  if( artboardStorageClone.config.designId != undefined){
      formData.append('id', artboardStorageClone.config.designId);
  }
  else
  {
  ...
  formData.append('name', artboardStorageClone.config.title);
  chrome.storage.sync.set({'waiting':true}); 
  for (let i = 0; i < event.variables.length; i++) {
    let variable = event.variables[i];
    artboardStorageClone = JSON.parse(artboardStorage);
    ...
    if (data.id) {
      chrome.runtime.sendMessage({type: "download", designId: artboardStorageClone.config.designId,fileName:variable['File Name'],message:event});
